// Render many spotlights on an image, passing the light definitions in
// global memory. This works, but is inefficient.
                           ...
                           lots_of_lights *lights)
{
  ...
  for (int i = 0; i < lights->count; i++) {
    brightness += light_brightness(x, y, width, height, lights->lights[i]);
  }
...
int main(int argc, char **argv)
{
  ...
  lots_of_lights *dev_lights;
  cudaCheckError(cudaMalloc(&dev_lights, sizeof(lots_of_lights)));
  cudaCheckError(cudaMemcpy(dev_lights, &lights, sizeof(lots_of_lights),
...

  {
    ...
    spotlights<<<grid_dim, BLOCK_DIM>>>(input2d, output2d, params.width,
                                        params.height, pitch, 0.0, dev_lights);
  }

  cudaCheckError(cudaFree(dev_lights));
